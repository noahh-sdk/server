...
impl ModVersion {
    ...
    pub async fn get_latest_for_mods(pool: &mut PgConnection, ids: &[&str], gd: Option<GDVersionEnum>) -> Result<HashMap<String, Vec<ModVersion>>, ApiError> {
        ...
        let mut query_builder: QueryBuilder<Postgres> = QueryBuilder::new(
            r#"SELECT
            mv.name, mv.id, mv.description, mv.version, mv.download_link, mv.hash, mv.noahh,
    ...
    pub async fn create_from_json(json: &ModJson, pool: &mut PgConnection) -> Result<(), ApiError> {
        ...
        builder.push("name, version, download_link, hash, noahh, early_load, api, mod_id) VALUES (");
