...
use crate::{AppData, types::{api::{ApiError, ApiResponse}, models::{mod_version::ModVersion, mod_entity::{download_noahh_file, Mod}}, mod_json::ModJson}};
...
pub async fn create_version(path: web::Path<CreateVersionPath>, data: web::Data<AppData>, payload: web::Json<CreateQueryParams >) -> Result<impl Responder, ApiError> {
    let mut pool = data.db.acquire().await.or(Err(ApiError::DbAcquireError))?;
    let mut file_path = download_noahh_file(&payload.download_url).await?;
